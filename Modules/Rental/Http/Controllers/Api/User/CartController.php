...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request)
    {
        ...
        $carts->module_id = getModuleId($request->header('moduleId'));
    ...
    {
        try {
            $provider_id = $this->cart->where('user_id', $user_id)->where('is_guest', $is_guest)->where('module_id', getModuleId($request->header('moduleId')))->first()?->provider_id;
            ...
            $response = match (true) {
                ...
                $this->cart->where('vehicle_id', $request->vehicle_id)->where('user_id', $user_id)->where('is_guest', $is_guest)->where('module_id', getModuleId($request->header('moduleId')))->exists() => [
                    'code' => 'cart_item',
    ...
    public function updateUserData(RentalCartUserData $user_data, Request $request)
    {
        ...
        $carts = $this->cart->where('user_id', $user_id)->where('is_guest', $is_guest)->where('module_id', getModuleId($request->header('moduleId')))->with(['vehicle'])->get();
    ...
    private function updateCartPrice($request, $user_id, $is_guest, $user_data = null)
    {
        ...
        $carts = $this->cart->where('user_id', $user_id)->where('is_guest', $is_guest)->where('module_id', getModuleId($request->header('moduleId')))
