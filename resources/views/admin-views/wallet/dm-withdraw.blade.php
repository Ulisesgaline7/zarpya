                            ...
                            @foreach ($withdraw_req as $k => $wr)
                                <tr>
                                    <td scope="row" class="px-3 py-4 fs-14 title-clr font-medium">
                                        {{ $k + $withdraw_req->firstItem() }}</td>

                                    <td class="px-3 py-4">
                                        <div class="fs-14 title-clr ">
                                            {{ \App\CentralLogics\Helpers::format_currency($wr['amount']) }}
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-3">
                                        <div>
                                            @if ($wr->deliveryman)
                                                <a href="{{ route('admin.users.delivery-man.preview', [$wr->deliveryman->id]) }}"
                                                    title="{{ $wr->deliveryman->f_name . ' ' . $wr->deliveryman->l_name }}"
                                                    class="d-flex  align-items-center gap-2">
                                                    <img width="40" height="40"
                                                        src="{{ $wr->deliveryman->image_full_url }}" alt="img"
                                                        class="w-40px h-40px rounded-circle">
                                                    <div class="info">
                                                        <div class="text-title fs-14">
                                                            {{ Str::limit($wr->deliveryman->f_name . ' ' . $wr->deliveryman->l_name, 20, '...') }}
                                                        </div>
                                                        <div class="color-334257B2 fs-12 font-light">
                                                            {{ $wr->deliveryman->phone }}
                                                        </div>
                                                    </div>
                                                </a>
                                            @else
...
@push('script_2')
    <script>
        ...
        $('.status-filter').on('change', function() {
            ...
            $.post({
                url: '{{ route('admin.transactions.delivery-man.status-filter') }}',
                data: {
